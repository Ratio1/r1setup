...
2. **Build the Collection**
   ...
   ```
   This will create a tarball like `ratio1-r1setup-1.0.0.tar.gz`

3. **Install the Collection**
   You can install the collection locally using:
   ```bash
   ansible-galaxy collection install ratio1-r1setup-1.0.0.tar.gz -p ./collections
   ```
...
3. **Publish**
   ```bash
   ansible-galaxy collection publish ./ratio1-r1setup-1.0.0.tar.gz --api-key=your_api_token
   ```
